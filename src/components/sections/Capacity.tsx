...
const CapacityBlock = ({ image, title, index }: CapacityBlockProps) => {
    ...
    return (
        ...
        >
            {/* IMAGE CONTAINER */}
            <div className="relative aspect-[4/3] w-full bg-white/5 rounded-2xl overflow-hidden border border-white/10 group-hover:border-primary-accent/40 transition-all duration-500 shadow-2xl">
                <img
                    src={image}
                    alt={title.replace(/\*/g, '')}
            ...
            <div className="px-2">
                <p className="text-xl md:text-2xl font-normal leading-tight text-foreground/90">
                    {parts.map((part, i) => (
                        i % 2 === 1 ? <strong key={i} className="text-primary-accent border-b border-primary-accent/30 font-medium">{part}</strong> : part
                    ))}
                </p>
            </div>
        </motion.div>
            ...
            <div className="container mx-auto max-w-6xl relative z-10">
                ...
                <div className="text-center mb-20 space-y-4">
                    <motion.h2
                        ...
                        viewport={{ once: true }}
                        className="text-4xl md:text-6xl lg:text-[64px] font-normal leading-tight"
                    >
